...
function Remove-WinGetApp {
    ...
    if ($script:Params.ContainsKey("User")) {
        Write-Host "正在为用户 $(Get-UserName) 添加卸载 $app 的计划任务..."
        Set-RunOnceWingetTask -appId $app
    }
    elseif ($script:Params.ContainsKey("Sysprep")) {
        Write-Host "正在为新用户添加卸载 $app 的计划任务..."
        Set-RunOnceWingetTask -appId $app
    }
}
...
function Get-AppRemovalMethod {
    ...
    if (-not $script:AppRemovalMethodCache) {
        ...
        try {
            ...
            Write-Warning "无法从 '$script:AppsListFilePath' 加载应用移除方式。未知应用将默认为 Appx 方式。错误：$_"
        }
    }
...
function Request-EdgeForceRemove {
    ...
    elseif ($(Read-Host -Prompt "是否要强制卸载 Microsoft Edge？不建议这样做！(y/n)") -eq 'y') {
        Write-Host ""
        Invoke-ForceRemoveEdge
    }
}
