...
function Invoke-RestartExplorer {
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 重启 Windows Explorer 进程" -ForegroundColor Cyan
        return
    }

    Write-Host "> 正在尝试重启 Windows Explorer 进程以应用所有更改..."

    if ($script:Params.ContainsKey('SkipExplorerRestart')) {
        Write-Host "已跳过 Explorer 进程重启，请手动重启电脑以应用所有更改" -ForegroundColor Yellow
        return
    }
    ...
    if ([Environment]::Is64BitProcess -eq [Environment]::Is64BitOperatingSystem) {
        Write-Host "正在重启 Windows Explorer 进程...（这可能导致屏幕闪烁）"
        Stop-Process -processName: Explorer -Force
    }
    else {
        Write-Host "无法重启 Windows Explorer 进程，请手动重启电脑以应用所有更改" -ForegroundColor Yellow
    }
}
